...
const HERD_SELECT: &str = r#"
    SELECT
        ...
        a.father_id,
        a.mother_id,
        (SELECT t2.tag_number FROM animals a2 JOIN tags t2 ON t2.id = a2.tag_id WHERE a2.id = a.father_id) AS father_tag_number,
        (SELECT t2.tag_number FROM animals a2 JOIN tags t2 ON t2.id = a2.tag_id WHERE a2.id = a.mother_id) AS mother_tag_number,
