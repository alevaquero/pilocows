...
// ── Popup geometry ────────────────────────────────────────────────────────────
// Centered on 480×800 (portrait). Box: 420×260, centered via lv_obj_center().
#define POPUP_W  420
#define POPUP_H  260
...
static void popup_show(const char *msg, lv_color_t bg_color, lv_color_t text_color)
{
    ...
    lv_obj_set_style_pad_all(box, 18, LV_PART_MAIN);
    ...
    lv_obj_set_width(lbl, POPUP_W - 36);
    lv_obj_set_style_text_font(lbl, &lv_font_app_24, LV_PART_MAIN);
    lv_obj_set_style_text_color(lbl, text_color, LV_PART_MAIN);
    lv_obj_align(lbl, LV_ALIGN_TOP_MID, 0, 0);

    // OK button
    lv_obj_t *btn = lv_btn_create(box);
    lv_obj_set_size(btn, POPUP_W - 36, 56);
    ...
    lv_label_set_text(btn_lbl, i18n_t(STR_BTN_OK));
    lv_obj_set_style_text_font(btn_lbl, &lv_font_app_24, LV_PART_MAIN);
