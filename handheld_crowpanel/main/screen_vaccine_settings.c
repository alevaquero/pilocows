...
static uint8_t s_id_buf[VACCINE_LIST_MAX];
...
static void rebuild_list(void) {
    ...
    int count = (int)vaccine_list(vlist, VACCINE_LIST_MAX);
...
static void on_add_confirm(const char *text, void *user_data) {
    ...
    if (text && text[0]) {
        uint8_t new_id = 0;
        esp_err_t err = vaccine_add(text, &new_id);
        if (err != ESP_OK) {
...
static void on_add_btn(lv_event_t *e) {
    ...
    ui_text_entry_cfg_t cfg = {
        .label = i18n_t(STR_VACCINE_NAME),
        .initial_text = "",
        .placeholder = NULL,
        .multiline = false,
        .password = false,
        .max_length = VACCINE_NAME_MAX - 1,
