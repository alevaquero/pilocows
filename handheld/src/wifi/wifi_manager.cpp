...
// ---------------------------------------------------------------------------
// Internal connection helper — reads credentials from NVS and connects.
// Safe to call from the WiFi event handler.
// ---------------------------------------------------------------------------
...
static void event_handler(void *arg, esp_event_base_t base, int32_t id, void *data)
{
    if (base == WIFI_EVENT) {
        if (id == WIFI_EVENT_STA_START) {
            ...
            // ESP-IDF returns records sorted by RSSI (strongest first)
            ...
            // Deduplicate by SSID — first occurrence is the strongest
...
{
    // Both calls are safe to call once — they return INVALID_STATE if already done
    esp_netif_init();
...
void wifi_set_credentials(const char *ssid, const char *pass)
{
    ...
    // Set config explicitly so the next connect uses the new credentials
    wifi_config_t cfg = {};
    ...
    s_retry = 0;
    esp_wifi_set_config(WIFI_IF_STA, &cfg);
    esp_wifi_disconnect();   // DISCONNECTED event fires but won't retry (s_retry=0 logic
                             // still applies; the explicit connect below takes over)
    esp_wifi_connect();
