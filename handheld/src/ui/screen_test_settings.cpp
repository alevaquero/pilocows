...
static void rebuild_list(void)
{
    ...
    int count = test_list(tlist, TEST_LIST_MAX);
    ...
    for (int i = 0; i < count; i++) {
        ...
        lv_label_set_text(lbl_del, i18n_t(STR_TEST_DELETE));
        ...
        static uint8_t id_buf[TEST_LIST_MAX];
        id_buf[i] = tid;
...
static void on_add_confirm(lv_event_t *e)
{
    ...
    if (name && name[0]) {
        uint8_t new_id = 0;
        esp_err_t err = test_add(name, &new_id);
        if (err != ESP_OK) {
...
void screen_test_settings_create(void)
{
    ...
    lv_label_set_text(s_lbl_btn_add, i18n_t(STR_TEST_ADD));
    ...
    lv_label_set_text(s_lbl_add_title, i18n_t(STR_TEST_NAME));
    ...
    lv_textarea_set_max_length(s_ta_name, TEST_NAME_MAX - 1);
